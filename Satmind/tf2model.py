...
class Critic(tf.keras.Model):
    def __init__(self, layer_1, layer_2, model_name='Critic',
        ...
        self.layer_1 = Dense(layer_1, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(l=0.01))
        self.layer_2 = Dense(layer_2, activation='relu',  kernel_regularizer=tf.keras.regularizers.l2(l=0.01))
...
class Actor(tf.keras.Model):
    def __init__(self, n_actions, action_bound, layer_1=64, layer_2=32, model_name='Actor',
        ...
        #                                                                          1 / np.sqrt(layer_1)))
        # self.layer_2 = tf.keras.layers.Dense(layer_2, activation=None,
        #                           kernel_initializer=tf.random.uniform(-1 / np.sqrt(layer_2),
        #                                                                            1 / np.sqrt(layer_2)),
        #                           bias_initializer=tf.random.uniform(-1 / np.sqrt(layer_2),
        #                                                                          1 / np.sqrt(layer_2)))
